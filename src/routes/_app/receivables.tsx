...
function Receivables() {
  ...
  const markPaid = async (id: string) => {
    const { error } = await supabase
      .from("receivables")
      .update({ status: "pago", received_at: new Date().toISOString().slice(0, 10) })
      .eq("id", id);
    if (error) return toast.error(error.message);
  ...
  const statusLabel = (s: string) =>
    ({ pendente: "Pendente", pago: "Recebido", atrasado: "Atrasado", cancelado: "Cancelado" }[s] ?? s);
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            A receber: <span className="font-semibold text-emerald-600">R$ {total.toFixed(2)}</span>
          </p>
        </div>
          ...
          {items.map((i) => (
            <div key={i.id} className="flex items-center justify-between rounded-xl border bg-card px-4 py-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`rounded px-2 py-0.5 text-xs font-medium ${statusColor(i.status)}`}>
                  {statusLabel(i.status)}
                </span>
                {i.status === "pendente" && (
                  <Button size="icon" variant="ghost" onClick={() => markPaid(i.id)} title="Marcar como recebido">
